...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., LoRA_dim=1024, prefix_dim=1024,
                 drop_rate_LoRA=0):
        super().__init__()
    ...
    def _attention(self, q, kv):
        B, N, C = q.shape
        N_qk = kv.shape[1]
        q = self.q(q).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k = self.k(kv).reshape(B, N_qk, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = self.v(kv).reshape(B, N_qk, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
    ...
    def _attention_weights(self, q, kv):
        B, N, C = q.shape
        N_qk = kv.shape[1]
        q = self.q(q).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k = self.k(kv).reshape(B, N_qk, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
    ...
    def _attention_aggreate(self, attn, v, reshape=True):
        B, H, _, C = v.shape

        attn = attn.softmax(dim=-1)      
        attn = self.attn_drop(attn)     # b, h, nq, v

        if reshape:
            x = (attn @ v).transpose(1, 2).reshape(B, -1, H * C)
        else:
            x = attn @ v

        return x

    def scaled_decoupled_attention_v1(self, v, p, scale=1.0, full_attention=False):
        N_v = v.shape[1]

        if full_attention:
            x = torch.cat([v, p], dim=1)
            x_v, attn = self._attention_weights(x, x)
        else:
            x_v, attn = self._attention_weights(v, torch.cat([v, p], dim=1))

        v_v = x_v[:, :, :N_v]
        p_v = x_v[:, :, N_v:]

        # ([64, 12, 197, 50]) 
        attn_v2v = attn[:, :, :N_v, :N_v]
        attn_v2p = attn[:, :, :N_v, N_v:]

        if full_attention:
            attn_p2vp = attn[:, :, N_v:]
            p2vp = self._attention_aggreate(attn_p2vp, torch.cat([v_v, p_v], dim=2))
            p_out = p2vp
        else:
            p_out = p

        v2v = self._attention_aggreate(attn_v2v, v_v)
        v2p = self._attention_aggreate(attn_v2p, p_v)

        v_out = (1 - scale) * v2v + scale * v2p
        return v_out, p_out, attn

    def forward(self, x, scale=0.1, attn_mask=None, full_attention=False):
        B, N, C = x.shape
        N_p = N - 197
        
        if N_p > 0:
            v, p = x[:, :-N_p], x[:, -N_p:]
            v, p, attn = self.scaled_decoupled_attention_v1(v, p, scale, full_attention=full_attention)
            x = torch.cat([v, p], dim=1)
        else:
            x, attn = self._attention(x, x)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, visual_prompt_dim=1024, LoRA_dim=1024,
                 adapter_dim=1024, prefix_dim=1024, drop_rate_LoRA=0, drop_rate_prompt=0, drop_rate_adapter=0):
        super().__init__()
    ...
    def set_sample_config(self, sample_LoRA_dim=None, sample_adapter_dim=None,
                          sample_prefix_dim=None, sample_last_prompt_tuning_dim=None, sample_prompt_tuning_dim=None):
    ...
    def forward(self, x, scale=0.1, attn_mask=None, full_attention=False):
        attn_x, attn = self.attn(self.norm1(x), scale=scale, attn_mask=attn_mask, full_attention=full_attention)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=PatchEmbed, norm_layer=None, progressive_add=False,
                 act_layer=None, weight_init='', super_prompt_tuning_dim=[], inter_prompt_share_dim=0, visual_prompt_indices=[], duplicate_prompt=False, super_LoRA_dim=1024,
                 super_adapter_dim=1024, super_prefix_dim=1024, drop_rate_LoRA=0, drop_rate_prompt=0,
                 drop_rate_adapter=0, IS_not_position_VPT=False):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                visual_prompt_dim=super_prompt_tuning_dim[i],
                LoRA_dim=super_LoRA_dim, adapter_dim=super_adapter_dim, prefix_dim=super_prefix_dim,
                drop_rate_LoRA=drop_rate_LoRA, drop_rate_prompt=drop_rate_prompt, drop_rate_adapter=drop_rate_adapter)
        ...
        self.visual_prompt_indices = visual_prompt_indices
        self.duplicate_prompt = duplicate_prompt
        self.progressive_add = progressive_add
        self.inter_prompt_share_dim = 0
        if super_prompt_tuning_dim[0] > 0:
            compound_prompts_visual = []
            for i in range(len(visual_prompt_indices)):
                prompt_dim = super_prompt_tuning_dim[i] if i==0 else super_prompt_tuning_dim[i] - inter_prompt_share_dim
                visual_prompt_token = nn.Parameter(torch.zeros(1, prompt_dim, embed_dim))
                nn.init.xavier_uniform_(visual_prompt_token)
                compound_prompts_visual.append(visual_prompt_token)

            if not IS_not_position_VPT:
                self.visual_prompt_token_pos_embed = nn.Parameter(torch.zeros(1, super_prompt_tuning_dim[0], embed_dim))
                nn.init.xavier_uniform_(self.visual_prompt_token_pos_embed)

            self.compound_prompts_visual = nn.ParameterList(compound_prompts_visual)

            self.semantic_share_prompt = nn.Parameter(torch.zeros(10, embed_dim))

            self.num_attribute_prompt = 2
            self.attribute_prompt = nn.Parameter(torch.zeros(self.num_attribute_prompt, 10, embed_dim))
            nn.init.kaiming_uniform_(self.semantic_share_prompt)
            nn.init.kaiming_uniform_(self.attribute_prompt)
        else:
            self.compound_prompts_visual = []
            self.semantic_share_prompt = None
    ...
    def freeze_stages(self):
        ...
        print('parameters:', total_para_nums, 'adapter', adapter_para_nums, 'LoRA', LoRA_para_nums, 'prompt',
              vp_para_nums, 'head', head_para_nums)
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        ...
        print('parameters:', total_para_nums, 'adapter', adapter_para_nums, 'LoRA', LoRA_para_nums, 'prompt',
              vp_para_nums, 'prefix', prefix_para_nums, 'head', head_para_nums, 'model', model_para_nums)

    def set_sample_config(self, config: dict):

        # visual prompt tuning
        self.sample_prompt_tuning_dim = config['visual_prompt_dim']
        # self.inter_share_length = config['inter_share_length']
        # print(config['visual_prompt_dim'])
        # print(config['inter_share_length'])     # [int]: 10
        ...
        for i, blocks in enumerate(self.blocks):
            ...
            blocks.set_sample_config(sample_prompt_tuning_dim=self.sample_prompt_tuning_dim[i],
                                     sample_LoRA_dim=self.sample_LoRA_dim[i],
                                     sample_prefix_dim=self.sample_prefix_dim[i],
                                     sample_last_prompt_tuning_dim=self.sample_prompt_tuning_dim[i - 1] if i > 0 else
    ...
    def get_sampled_params_numel(self, config):
        ...
        if self.sample_prompt_tuning_dim[0] != 0:
            for visual_prompt_token in self.compound_prompts_visual:
                sample_prompt_tuning_param += visual_prompt_token.numel()

            if not self.IS_not_position_VPT:
                sample_prompt_tuning_param += self.visual_prompt_token_pos_embed.numel()

        return sum(numels) + sample_prompt_tuning_param

    @staticmethod
    def calculate_matching_loss(x, attn):
        '''
        input:
            source: (batch, length_1, dim)
            target: (batch, length_2, dim)
        return:
            cost: matching cosine loss
        '''
        B, N, C = x.shape
        N_p = N - 197
        if N_p == 0:
        ...
        num_top_k = 10
        attn = attn[:, :, 0, :197].mean(1)
        indices = torch.topk(attn, k=num_top_k, dim=-1)[1]
        v = v[torch.arange(x.shape[0]).unsqueeze(1).expand(-1, num_top_k), indices]

        p2v_sim = F.cosine_similarity(p.unsqueeze(2), v.unsqueeze(1), dim=-1)
        cost_matrix = 1 - p2v_sim

        cost, min_indices = torch.min(cost_matrix, dim=2)
        # print(min_indices)
        return cost.mean()

    @staticmethod
    def select_inter_semantic_shared_token(attention, target, length):
        '''
        input:
            attention: (batch, prompt_length)
            target: (batch, prompt_length, dim)
            length: select prompt length
        return:
            topk_token: (batch, length, dim)
        '''
        dim = target.shape[2]

        topk_indices = torch.topk(attention, k=length, dim=1, sorted=False)[1]      # (b, length)
        topk_indices = topk_indices.unsqueeze(-1).expand(-1, -1, dim)
        topk_token = torch.gather(target, 1, topk_indices)

        return topk_token

    def construct_prompt(self, semantic_prompt, attribute_prompt=None, semantic_share_prompt=None):
        final_prompt = semantic_prompt
        
        if attribute_prompt is not None:
            final_prompt = torch.cat([final_prompt, attribute_prompt], dim=1)

        if semantic_share_prompt is not None:
            final_prompt = torch.cat([final_prompt, semantic_share_prompt], dim=1)
        
        return final_prompt

    def consturct_attribute_prompt(self, x, attn, scale=0.1, counter=0):
        B, N, C = x.shape

        attribute_prompt = self.attribute_prompt[counter].expand(B, -1, -1)
        K = attribute_prompt.shape[1]

        attn = attn[:, :, 0, :197].mean(1)
        indices = torch.topk(attn, k=K, dim=-1)[1]
        x = x[torch.arange(x.shape[0]).unsqueeze(1).expand(-1, K), indices]

        feature_bank = self.feature_bank    # (200, 768)
        feature_bank = feature_bank.expand(B, -1, -1)

        sim = F.cosine_similarity(x.unsqueeze(2), feature_bank.unsqueeze(1), dim=-1) # (B, 10, 200)
        sim = torch.softmax(sim, dim=-1)
        pattern_feature = sim @ feature_bank
        out = scale * pattern_feature + (1 - scale) * attribute_prompt
        return out

    def forward_features(self, x, use_matching_loss=False):
        B = x.shape[0]
        ...
        if self.semantic_share_prompt is not None:
            semantic_share_prompt = self.semantic_share_prompt.expand(B, -1, -1)
        else:
            semantic_share_prompt = None
        ...
        if self.sample_prompt_tuning_dim[0] != 0:
            visual_prompt = compound_prompts_visual[0].expand(B, -1, -1)

            if self.semantic_share_prompt is not None:
                visual_prompt = torch.cat([visual_prompt, semantic_share_prompt], dim=1)
                x = torch.cat((x, visual_prompt), dim=1)
        
        # prompt_scale = F.sigmoid(self.prompt_scale)
        # prompt_scale = self.lamda
        ...
        for i in range(len(self.blocks)):
            if i in self.visual_prompt_indices[-self.num_attribute_prompt:]:
                # not suitable for i==0
                attribute_prompt = self.consturct_attribute_prompt(x, attn, counter=ap_counter)
                ap_counter += 1
            else:
                attribute_prompt = None

            if i in self.visual_prompt_indices and i > 0:
                visual_prompt = compound_prompts_visual[counter].expand(B, -1, -1)
                visual_prompt = self.construct_prompt(visual_prompt, attribute_prompt, semantic_share_prompt)
                x = torch.cat([x[:, :N], visual_prompt], dim=1)
                counter += 1
            else:
                if self.semantic_share_prompt is not None:
                    x = torch.cat([x[:, :-10], semantic_share_prompt], dim=1)
            ...
            x, attn = self.blocks[i](x, scale=0.1, full_attention=full_attention)

            # prompt loss
            if i >= 11:
                matching_loss.append(self.calculate_matching_loss(x, attn))

            with torch.no_grad():
                attn[:, :, 0, :N].mean(1)

            intermidiate_attn.append(attn)  # (64, 247, 768)
            intermidiate_outputs.append(x)  # (64, 247, 768)

        x = self.norm(x)

        if len(matching_loss) > 0 and self.training:
            matching_loss = sum(matching_loss) / len(matching_loss)
        else:
            matching_loss = 0.

        if self.dist_token is None:
            return self.pre_logits(x[:, 0]), intermidiate_outputs, intermidiate_attn, matching_loss
        else:
            return x[:, 0], x[:, 1]

    def forward(self, x, use_matching_loss=False):
        x, intermidiate_outputs, intermidiate_attns, matching_loss = self.forward_features(x, use_matching_loss)
        ...
        if self.training:
            return x, intermidiate_outputs, matching_loss
        else:
            return x, intermidiate_outputs, intermidiate_attns
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    for i, block in enumerate(model.blocks.children()):
        ...
        block.attn.q.weight.copy_(_n2p(w[f'{mha_prefix}query/kernel'], t=False).flatten(1).T)
        block.attn.q.bias.copy_(_n2p(w[f'{mha_prefix}query/bias'], t=False).reshape(-1))
        block.attn.k.weight.copy_(_n2p(w[f'{mha_prefix}key/kernel'], t=False).flatten(1).T)
        block.attn.k.bias.copy_(_n2p(w[f'{mha_prefix}key/bias'], t=False).reshape(-1))
        block.attn.v.weight.copy_(_n2p(w[f'{mha_prefix}value/kernel'], t=False).flatten(1).T)
        block.attn.v.bias.copy_(_n2p(w[f'{mha_prefix}value/bias'], t=False).reshape(-1))
